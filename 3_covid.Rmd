...
```{r}
node_colors <- c(
  "lymphocyte of B lineage " = "gray80",           
  "antibody secreting cell " = "#D73027",           
  "long lived plasma cell " = "#FC8D59",           
  "IgA plasma cell" = "#FEE090",                   
  "IgM plasma cell" = "#FEC44F",                  
  "B cell, CD19-positive " = "#4575B4",             
  "mature B cell " = "#3288BD" ,                     
  "naive B cell" = "#74ADD1",                     
  "immature B cell" = "#1C9099",                   
  "memory B cell " = "#91BFDB",                     
  "class switched memory B cell" = "#66C2A5",      
  "unswitched memory B cell" = "#41B6C4",          
  "IgG plasma cell" = "#FDAE61",                   
  "plasmablast" = "#E34A33"                       
)

nodeAttrs <- list(fillcolor = node_colors)

plot(pan.gr1,
     nodeAttrs = nodeAttrs,
     attrs = list(
       node = list(fontsize = 27, shape = "box", style = "filled",
                   fixedsize = FALSE)
     ))
...
set.seed(1312)
cal <- sample(1:dim(other.bal)[2], 1000)
df.train <- df[-cal,]
df.cal <- df[cal,]
...
rownames(spe_cal) <- Hs_mapping$ensembl_id[match(rownames(spe_cal),
                                                       Hs_mapping$symbol)]
is_bad <- is.na(rownames(spe_train))
spe_train <- spe_train[!is_bad, ]
is_bad <- is.na(rownames(test))
test <- test[!is_bad, ]
is_bad <- is.na(rownames(spe_cal))
spe_cal <- spe_cal[!is_bad, ]
# Train on reference
...
pr.cal <- apply(cal.pred, 1, function(i) names(i)[which.max(i)])
set.seed(1133)
cal_oracle <- resample.oracle(df.cal, df.test, cal.pred, seed=1)

oracle_sets <- getPredictionSets(x_query=test.pred, x_cal=cal_oracle$p.cal,
                          y_cal=cal_oracle$cal$Y,
                          alpha=0.1, follow_ontology = FALSE, resample=FALSE,
                          return_sc=FALSE, onto=onto)
mean(mapply(`%in%`, test$cell_type_curated, oracle_sets))
#0.910
oracle_sets_gr <- getPredictionSets(x_query=test.pred, x_cal=cal_oracle$p.cal,
                          y_cal=cal_oracle$cal$Y,
                          alpha=0.1, follow_ontology = TRUE, resample=FALSE,
...
for(i in seq_along(test$comm_anc_GI))
  ch[i] <- length(scConform:::.children(test$comm_anc_GI[[i]], onto)) == length(test$pr_crc_res1[[i]])
sum(!ch)
...
pr.class <- apply(test.pred, 1, function(i) names(i)[which.max(i)])
test$pr_label <- pr.class
mean(test$cell_type_curated==test$pr_label)
...
p1 <- plotTSNE(test, color_by = "pr_label") +
    ...
    labs(color = "Point predictions")+ggtitle("Point predictions")+
    theme(legend.position = "none")
p2 <- plotTSNE(test, color_by = "comm_anc_GI") +
    scale_color_manual(values = node_colors) +
    labs(color = "Graph-based labels")+
    theme(legend.position = "none")+ggtitle("Graph-based sets")
...
Plot with cells colored by correct/wrong label

```{r}
test$YN_pp <- ifelse(test$cell_type_curated==test$pr_label, "Yes", "No")

test$YN_crc <- ifelse(mapply(`%in%`, test$cell_type_curated, test$pr_crc_res1),
                      "Yes", "No")

p3 <- plotTSNE(test, color_by = "YN_pp") +
  scale_color_manual(
    values = c(
      "Yes" = "#7F7F7F",   # gray (colorblind safe)
      "No"  = "#D55E00"    # blue (from the Okabe–Ito palette)
    ),
    name = "correct"
  ) +
  ggtitle("Point predictions")

p4 <- plotTSNE(test, color_by = "YN_crc") +
  scale_color_manual(
    values = c(
      "Yes" = "#7F7F7F",   # gray (colorblind safe)
      "No"  = "#D55E00"    # blue (from the Okabe–Ito palette)
...
```{r}
tab <- prop.table(table(test$comm_anc_GI, test$cell_type_curated), 2)

cols <- node_colors[rownames(tab)]
...
bp <- barplot(
  ...
  col = cols,
  legend = FALSE,
  ylab = "Predicted ancestor",
  xlab = "True cell types"
)
...
colnames(df) <- c("comm_anc_GI", "cell_type_curated", "prop")
df$comm_anc_GI1 <- factor(df$comm_anc_GI,
                         levels = c("lymphocyte of B lineage ",
                                    "antibody secreting cell ",
                                    "B cell, CD19-positive ",
                                    "long lived plasma cell ",
                                    "mature B cell ",
                                    "immature B cell",
                                    "IgA plasma cell", "IgM plasma cell",
                                    "IgG plasma cell", "plasmablast",
                                    "naive B cell",
                                    "memory B cell ",
                                    "class switched memory B cell",
                                    "unswitched memory B cell"))

df$cell_type_curated <- factor(
  df$cell_type_curated,
  levels = c(
    "class switched memory B cell",
    "unswitched memory B cell",
    "naive B cell",
...
library(stringr)
p.bar <- ggplot(df, aes(x = cell_type_curated, y = prop, fill = comm_anc_GI1)) +
  geom_bar(stat = "identity") +
   scale_x_discrete(labels = function(x) str_wrap(x, width = 17)) +
  scale_fill_manual(
    values = node_colors,
    name = "Predicted ancestor"
  ) +
  ylab("Predicted ancestor") +
  xlab("True cell types") +
  theme_bw() +
...
# distinct_colors <- createPalette(n, seedcolors = c("#7F7F7F"))
# 
# node_colors_new <- setNames(distinct_colors, names(node_colors))
# ggplot(df, aes(x = cell_type_curated, y = prop, fill = comm_anc_GI)) +
#   geom_bar(stat = "identity") +
#   scale_fill_manual(
#     values = node_colors_new,
#     name = "Predicted ancestor"
#   ) +
#   ylab("Predicted ancestor") +
#   xlab("True cell types") +
...
#   )
# 
# nodeAttrs <- list(fillcolor = node_colors_new)
# plot(pan.gr1,
#      nodeAttrs = nodeAttrs,
#      attrs = list(
#        node = list(fontsize = 27, shape = "box", style = "filled",
