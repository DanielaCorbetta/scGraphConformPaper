...
# Graph-based (simple)
sets_graphs_s <- getPredictionSets(p_test, p_cal, df_cal$Y, follow_ontology = TRUE,
                             resample = FALSE, return_sc = FALSE, onto = opi2,
                             labels = unique(df_cal$Y),
                             BPPARAM = MulticoreParam(workers = 4),
                             method = .predSetsSimple, lambdas = seq(0.001, 1, length.out = 100))

# Check coverage
(cvg_graphs_s <- mean(mapply(function(vec, lst) vec %in% lst, df_test$Y, sets_graphs_s)))

# Graph-based (steps from the pr class)
sets_graphs_step <- getPredictionSets(p_test, p_cal, df_cal$Y, follow_ontology = TRUE,
                             resample = FALSE, return_sc = FALSE, onto = opi2,
                             labels = unique(df_cal$Y),
                             BPPARAM = MulticoreParam(workers = 4),
                             method = "step", lambdas = c(0:5))

# Check coverage
(cvg_graphs_step <- mean(mapply(function(vec, lst) vec %in% lst, df_test$Y, sets_graphs_step)))
lstep <- sapply(sets_graphs_step, length)
l <- sapply(sets_graphs, length)

# Rank leaves and cum prob
sets_graphs_rank <- getPredictionSets(p_test, p_cal, df_cal$Y, follow_ontology = TRUE,
                             resample = FALSE, return_sc = FALSE, onto = opi2,
                             labels = unique(df_cal$Y),
                             BPPARAM = MulticoreParam(workers = 4),
                             method = "rank", lambdas = seq(0.001, 1, length.out = 100))

# Check coverage
(cvg_graphs_rank <- mean(mapply(function(vec, lst) vec %in% lst, df_test$Y, sets_graphs_rank)))
lrank <- sapply(sets_graphs_rank, length)
mean(lrank)
mean(l)
